...
export default function FeaturedPropertyList() {
  ...
  const [FeaturedPropertyList, setFeaturedPropertyList] = useState([]);

  useEffect(() => {
    const fetchFeaturedProperties = async () => {
      const data = await supabaseApi.getWithFilter("properties", {
        featured: true,
      });
      setFeaturedPropertyList(data || []);
    };
  ...
  return (
    <div className="w-full flex flex-col py-8 px-2 md:px-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 py-8 px-4 md:px-8 w-full">
        {FeaturedPropertyList.slice(0, 3).map((property) => (
